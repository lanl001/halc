	...
	for (it = ppath.begin(); it != ppath.end(); it++)
	{
		...
		for (it2 = it->end() - 1; it2 >= it->begin(); it2--)
		{
			if ((it2 - 1) >= it->begin())
			{
				...
				else if (ispositive && undigraph.graph[temp][pair<unsigned long, unsigned long>(jindexofsubcontig, iindexofsubcontig)] <= max_support / 100)
				{
					sum.back().second = true;
				}
				else if (!ispositive && undigraph.graph[temp][pair<unsigned long, unsigned long>(iindexofsubcontig, jindexofsubcontig)] <= max_support / 100)
				{
					sum.back().second = true;
				}
